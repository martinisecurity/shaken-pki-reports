# STIR/SHAKEN Certificate Repository Compliance

## Unknown

- 4 repositories were included in the corpus being tested
- 0 repositories in the corpus were skipped because they were duplicated
- 4 repositories being tested against the remaining rules
- 3.00 issues on average found in non-compliant certificate repository URLs
- 100.00% of repositories contain one or more Error level issue
- 75.00% of repositories contain one or more Warning level issue
- 0.00% of repositories contain one or more Notice level issue
- 7868ms average time it took to download each certificate

| Instances | Test | Source |
|-----------|------|--------|
| 3 | [e_atis_cache_header](ISSUES/e_atis_cache_header/README.md) | ATIS-1000074 |
| 1 | [e_bad_url](ISSUES/e_bad_url/README.md) | System |
| 2 | [e_http_status_200](ISSUES/e_http_status_200/README.md) | HTTP |
| 3 | [w_atis_content_type](ISSUES/w_atis_content_type/README.md) | ATIS-1000080 |
| 3 | [w_atis_pem_certificate_chain](ISSUES/w_atis_pem_certificate_chain/README.md) | ATIS-1000080 |

| Repository | Problems | Link |
|------------|----------|------|
| `https://app.connexcs.com/api/stir-shaken/cert/41.crt` | true | [view](REPOS/cc2f8a40c2bfec30ddc6c0b45b770990ddcae1d7/README.md) |
| `https://dial.maqssolution.live/255K.pem` | true | [view](REPOS/adf028fff3c5bfdb49037b7d051913539200fab7/README.md) |
| `https://shaken.spectrum.com/4d65efdb8a1ca366e9576c8fda747fa4.pem` | true | [view](REPOS/18cdd5acaf4706a8bff59aa0338843318ac4e3c1/README.md) |
| `https://ssc.fractel.net/ssc/fractelssc.pem` | true | [view](REPOS/43cef8ecc75c42a157f4a770680c7bc638e02617/README.md) |


Generated: 10 Nov 22 23:30 UTC