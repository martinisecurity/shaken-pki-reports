# STIR/SHAKEN CA Ecosystem Compliance

## GBSDTech
Name: e_sti_ca_authority_key_identifier\
Source: ATIS-1000080\
Citation: ATIS-1000080.v003 / 6.4.1 SHAKEN Certificate Requirements\
Effective Date: 04 Sep 20 00:00 UTC\
Description: STI certificates shall contain an Authority Key Identifier extension

### Leaf Certificates

| Status | Subject | Link | Details |
|--------|---------|------|---------|

no warning, or error, or not effective date level issues were found

### CA Certificates

| Status | Subject | Link | Details |
|--------|---------|------|---------|
| error | CN=1RouteGroup SHAKEN Intermediate CA, O=1RouteGroup, ST=Texas, C=US | [view](../../CERTIFICATES/b34acd5cf741f6c98726c200f39517c4bd02d4cd/README.md) | STI certificates shall contain an Authority Key Identifier extension |


Generated: 28/10/2022 at 18:55:00